...
#	./Xilinx_Vivado_SDK_Web_2018.2_0614_1954 --noexec --keep --nox11 --target vivado_tmp
#	Creating directory vivado_tmp
#
# Source configuration information for a v2018.2 Vivado Image Build
...
if [ -f $XLNX_VIVADO_BATCH_CONFIG_FILE ] || [ -L $XLNX_VIVADO_BATCH_CONFIG_FILE ]; then
	echo "Xilinx Batch Mode Configuration File: [Good] "$XLNX_VIVADO_BATCH_CONFIG_FILE
else
	# File does not exist
	echo "ERROR: Xilinx Batch Mode Configuration File: [Missing] "$XLNX_VIVADO_BATCH_CONFIG_FILE
	exit $EX_OSFILE
fi
...
echo "-----------------------------------"
echo " - Install dependencies and download Vivado Installer and configuration file into container..."
echo "-----------------------------------"
...
docker exec -it $DOCKER_CONTAINER_NAME \
	bash -c "if [ ${BUILD_DEBUG} -ne 0 ]; then set -x; fi \
	&& cd ${HOME_DIR}/downloads/tmp \
	&& ${XLNX_VIVADO_WEB_INSTALLER} --noexec --nox11 --target unified_tmp \
	&& cd unified_tmp \
...
echo "-----------------------------------"
ln -s ../$GENERATED_DIR/$XLNX_VIVADO_OFFLINE_INSTALLER $XLNX_VIVADO_OFFLINE_INSTALLER
